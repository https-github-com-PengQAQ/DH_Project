...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<StateType>(translator.PushStateType, translator.Get, translator.UpdateStateType);
        ...
        int StateType_TypeID = -1;
		int StateType_EnumRef = -1;
        
        public void PushStateType(RealStatePtr L, StateType val)
        {
            if (StateType_TypeID == -1)
            {
			    bool is_first;
                StateType_TypeID = getTypeId(L, typeof(StateType), out is_first);
				
				if (StateType_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(StateType));
				    StateType_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, StateType_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, StateType_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for StateType ,value="+val);
            }
			
			LuaAPI.lua_getref(L, StateType_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out StateType val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != StateType_TypeID)
				{
				    throw new Exception("invalid userdata for StateType");
				}
                ...
                {
                    throw new Exception("unpack fail for StateType");
                }
				val = (StateType)e;
            ...
            {
                val = (StateType)objectCasters.GetCaster(typeof(StateType))(L, index, null);
            }
        }
		
        public void UpdateStateType(RealStatePtr L, int index, StateType val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for StateType ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			{
			    StateType[] array = obj as StateType[];
				translator.PushStateType(L, array[index]);
				return true;
			}
